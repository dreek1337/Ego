...
from src_posts.application import PostService, UpdatePostData, UserIsNotPostCreator
...
@pytest.mark.asyncio
async def test_correct_update_post(uow: PostUoWMock, mapper: MapperImpl) -> None:
    ...
    data = UpdatePostData(
        post_id="gl5MJXMBMk1dGnErnBW8", creator_id=0, text_content="Test update post!"
    )
...
async def test_deleted_post_not_found(uow: PostUoWMock, mapper: MapperImpl) -> None:
    ...
    data = UpdatePostData(
        post_id="gl5MJXMBMk1dGnErnBW81", creator_id=0, text_content="Test update post!"
    )
...
async def test_user_is_not_post_creator_to_delete(
    ...
    data = UpdatePostData(
        post_id="gl5MJXMBMk1dGnErnBW8", creator_id=1, text_content="Test update post!"
    )
