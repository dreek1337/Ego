...
from src_posts.application import CreatePostData, PostService
...
@pytest.mark.asyncio
async def test_create_post(uow: PostUoWMock, mapper: MapperImpl) -> None:
    post_service = PostService(uow=uow, mapper=mapper)

    post_data = CreatePostData(creator_id=0, text_content="My test post!")
