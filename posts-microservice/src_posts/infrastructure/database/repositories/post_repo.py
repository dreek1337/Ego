from src_posts.application import ElasticIndexes
from src_posts.application.posts.dto import PostDTO
from src_posts.application.posts.interfaces import GetPostsFilters, PostReader, PostRepo
from src_posts.domain import Empty, PostAggregate, PostId
from src_posts.infrastructure.database.repositories.base import ElasticPostRepoBase
...
class PostReaderImpl(ElasticPostRepoBase, PostReader):
    ...
    async def get_posts_by_creator_id(
        ...
        query = {
            ...
            "sort": [{"created_at": {"order": filters.order}}],
        }
        ...
        async with self._engine() as connection:
            posts = await connection.search(
                index=ElasticIndexes.POST_MS.value, body=query
            )
    ...
    async def full_text_posts_search(
        ...
        """
        query = {
            "query": {"match_phrase": {"text_content": query_string}},
            "sort": [{"created_at": {"order": filters.order}}],
        }
    ...
    async def get_post_by_id(self, post_id: PostId) -> PostAggregate:
        ...
        async with self._engine() as connection:
            post = await connection.get(
                index=ElasticIndexes.POST_MS.value, id=post_id.get_value
            )
    ...
    async def create_post(self, data: PostAggregate) -> PostAggregate:
        ...
        async with self._engine() as connection:
            created_data = await connection.index(
                index=ElasticIndexes.POST_MS.value, document=document
            )
